  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const stockInRows: StockInRow[] = mutRes.data
    ...
    try {
      const res = await apiPost<BackendResponse<BackendMutation>>(
        "/stock/in",
        {
          ...
          catatan: data.notes,
          tanggal: data.date,
          nomor_batch: data.batchNumber,
          tanggal_kedaluwarsa: data.expiryDate,
        },
        token
      );

      if (!res.success) {
        throw new Error(res.message || "Gagal mencatat stock IN");
      }
      ...
      toast({
        title: "Stock IN berhasil dicatat",
        description: "Mutasi stok masuk berhasil disimpan.",
      });
    } catch (error) {
      toast({
        title: "Gagal mencatat stock IN",
        description:
          error instanceof Error ? error.message : "Terjadi kesalahan",
        variant: "destructive",
      });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Catat mutasi stok masuk</p>
        </div>
